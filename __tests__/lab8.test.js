describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    let allButtonsRemoved = true;

    for(let productItem of productItems){
      let shadowRootHandle = await productItem.getProperty('shadowRoot');
      let shadowRoot = await shadowRootHandle.asElement();
      let button = await shadowRoot.$('button');
      let buttonText = await page.evaluate((btn) => btn.innerText, button);

      if (buttonText !== 'Remove from Cart'){
        allButtonsRemoved = false;
        break;
      }
    }
    // Also check to make sure that #cart-count is still 20
    let cartCount = await page.$eval('#cart-count', (element) => element.innerText);
    expect(allButtonsRemoved).toBe(true);
    expect(cartCount).toBe('20');
  }, 10000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let productItem of productItems){
      ...
      expect(allButtonsAdded).toBe(true);
      expect(cartCount).toBe('0');
  }, 10000);
  ...
  it('Checking the localStorage to make sure cart is correct', async () => {
    ...
    let cartIsEmpty = JSON.parse(cartValue).length === 0;
